...
- `CortexIndex` is now a versioned Rust/Swift schema persisted in SQLite and returned by compile. It ties corpus hash, source refs, artifact IDs, region sketches, route examples, and the legacy `MemoryMap` compatibility projection into one current cortex contract. Runtime query and chat memory search now load the persisted current index and use its sketches, route examples, source refs, semantic centroid scores, attention marks, and access signals before falling back to legacy map-only regions. Remaining drawback: model selection still does not load or gate on the fresh adapter, and trace-only routing still uses the compatibility `Router` path.

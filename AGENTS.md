...
- Attention marks now influence direct memory search and chat planner memory search for chunk/document/region targets. Pin/promote/active marks lift relevant source recall, decay/suppress marks lower ranking without making old material unreachable. Remaining drawback: ranking still does not yet use freshness, access history, source trust, explicit hot/warm/cold state beyond transcript hotness, active project/session context, or time decay.

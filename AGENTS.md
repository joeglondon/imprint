...
- `CortexIndex` is now a versioned Rust/Swift schema persisted in SQLite and returned by compile. It ties corpus hash, source refs, artifact IDs, region sketches, route examples, and the legacy `MemoryMap` compatibility projection into one current cortex contract. Remaining drawback: query planning and model selection still mostly read the legacy map/artifacts instead of loading the persisted `CortexIndex` as their primary contract.

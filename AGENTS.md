...
- `CortexIndex` is now a versioned Rust/Swift schema persisted in SQLite and returned by compile. It ties corpus hash, source refs, artifact IDs, region sketches, route examples, and the legacy `MemoryMap` compatibility projection into one current cortex contract. Runtime query, chat memory search, and CLI trace routing now load the persisted current index and use its sketches, route examples, source refs, semantic centroid scores, attention marks, and access signals before filling with legacy map-only regions. Remaining drawback: model selection still does not load or gate on the fresh adapter, and the public `Router` trait remains a compatibility-map API.
- Adapter freshness state is now reported by `memory_compile`/CLI/MCP JSON, persisted in SQLite, exposed through FFI, shown in the Swift model inspector, and refreshed after import/rebuild. It is not yet used for model selection/loading.
- Cortex compile now automatically prepares an MLX-compatible adapter dataset when the current training exports have no fresh manifest. This writes `train.jsonl`, `valid.jsonl`, `test.jsonl`, and an inspectable `adapter_manifest.json` under `store/adapters/prepared-<source-hash>/`, using the configured compiler model. Import/rebuild now trigger compile/prepare after source memory is saved. Remaining drawback: this is preparation only; real MLX LoRA training is still manual through `training/train_mlx_lora.py`.
- MLX cortex adapter dataset preparation is now a core import/rebuild step, but real MLX LoRA training is not yet a core import/rebuild step.
...
- Routing now returns a structured `RoutePlan` with candidate regions, scores, matched terms, and suggested next tool steps. Runtime query execution and CLI trace now blend cortex sketches, lexical matches, embedding similarity against region centroids, attention marks, and recent access signals so semantically relevant regions can be selected even when compact map wording is weak. Remaining drawback: centroid scoring is still coarse, and the system still needs a stronger recursive-agent policy contract.
...
- Attention marks, recent access history, source metadata, and active chat-session context now influence direct memory search and chat planner memory search for chunk/document/region targets. Pin/hot/promote/warm/active marks lift relevant source recall, while cold/decay/suppress marks lower ranking without making old material unreachable. Query hits, opens, source expansions, and anchor jumps are persisted as `memory_accesses` and feed retrieval as weak decayed attention signals. Retrieval also applies a bounded source-quality signal from document/chunk metadata such as `source_type`, `source_trust`, `confidence`, `retrieved_at`, and `freshness_expires_at`, so trusted local sources and fresh confident findings can outrank stale or low-confidence derived evidence when base relevance is close. Active `ChatSession` attention marks now boost chat-derived chunks carrying the matching `chat_session_id`, making the current conversation a real retrieval context instead of only an audit event. Remaining drawback: ranking still does not yet use active project/library workspace context, learned attention, explicit user-visible attention controls, or a mature time-decay policy.
...
- 2026-05-06: Import and rebuild now refresh cortex artifacts and prepared adapter manifests immediately after saving source memory, while filtering old compiler-generated documents out of the source rebuild before regenerated artifacts are synced back. `ImportResult` now returns the adapter state and the Swift app displays fresh/stale state after import or rebuild. Remaining drawback: the refresh is synchronous and prepares data only; it does not queue background LoRA training or load the adapter into chat/model routing.
- 2026-05-06: Source recall now has an attention-aware ranking path. `MemoryQueryEngine::execute_with_attention` applies persisted pin/promote/active/decay/suppress marks to region routing and chunk hit ordering, and `run_query` plus chat memory searches load those marks before retrieval. Added a regression test proving a pinned chunk can become the preferred recall address. Remaining drawback: attention is a simple deterministic score adjustment, not yet a full learned attention/ranking policy with freshness, source trust, access history, or decay curves.
- 2026-05-06: Added persisted memory access history for source recall. Top query hits, routed regions, opened nodes, expanded chunks, and anchor jumps now write `memory_accesses` records with actor, reason, target kind, and access kind. `MemoryQueryEngine::execute_with_signals` blends those recent access records into retrieval/routing as weak decayed attention, and chat planner memory searches now use the same signal. Added a regression test proving recent access history can prefer a previously expanded/opened chunk. Remaining drawback: access ranking is deterministic and local to a 30-day window; it is not yet a learned attention policy and does not yet distinguish source trust or active project/session context.
- 2026-05-06: Source recall now uses bounded source-quality metadata during ranking. `MemoryQueryEngine::execute_with_signals` blends source type, optional `source_trust`, confidence, retrieved time, and expiration metadata into chunk scoring after vector/lexical relevance, explicit attention marks, and recent access signals. Added a regression test proving trusted local source memory can outrank stale low-confidence web-derived evidence when the content relevance is otherwise tied. Remaining drawback: trust metadata is still manually supplied/ad hoc, not a normalized persisted source policy with user-visible controls, source-family defaults, or learned calibration.
- 2026-05-06: Added a first-class persisted `CortexIndex` schema. `compile_memory_brain` now builds and saves `cortex-index:current` with schema version, source corpus hash, source refs, artifact IDs, per-region sketches, route examples, and a legacy `MemoryMap` projection; Swift mirror models and the compile regression test were updated.
- 2026-05-06: Runtime query and chat retrieval now consult the persisted `CortexIndex` as the primary route contract. `MemoryQueryEngine` scores cortex region sketches using labels, summaries, route examples, source refs, semantic region centroids, explicit attention marks, and recent access history, then appends legacy map-only fallback regions so derived compiler artifacts remain searchable. Added a regression test proving cortex route examples can override stale legacy map wording. Remaining drawback: the public `Router` trait and trace-only route helper remain compatibility-map based, and model selection still does not use adapter freshness.
- 2026-05-06: CLI trace routing now uses the same cortex-aware signal path as runtime query routing. `MemoryQueryEngine::trace_with_signals` embeds the trace query, scores the current `CortexIndex` with semantic centroid, attention, and access signals, and reports route next steps for source recall. Legacy map-only candidates now fill gaps instead of re-sorting above positive cortex candidates. Added a regression test proving trace follows cortex route examples over stale compatibility-map wording. Remaining drawback: the public `Router` trait remains compatibility-map based, and model selection still does not use adapter freshness.
- 2026-05-06: Added first-class hot/warm/cold attention actions for source recall. MCP clients and Swift mirror models can now write these marks for any attention target, the chat planner prompt can emit them, and retrieval ranking blends them alongside pin/promote/active/decay/suppress marks. Added a regression test proving a hot chunk is preferred over a cold chunk when source relevance is otherwise tied. Remaining drawback: there is still no user-facing attention control surface or learned policy for setting hot/warm/cold automatically from project/session context.
- 2026-05-06: Active chat sessions now influence source recall directly. `MemoryQueryEngine` maps `ChatSession` attention marks through chat-derived document/chunk `chat_session_id` metadata, so the current conversation can prefer its own transcript memory when relevance is otherwise tied. Added a regression test proving an active session mark wins over an older session. Remaining drawback: this is still session-local metadata boosting, not a full active project/workspace context model or learned attention policy.

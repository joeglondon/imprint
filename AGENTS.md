...
- Attention marks, recent access history, and source metadata now influence direct memory search and chat planner memory search for chunk/document/region targets. Pin/promote/active marks lift relevant source recall, decay/suppress marks lower ranking without making old material unreachable. Query hits, opens, source expansions, and anchor jumps are persisted as `memory_accesses` and feed retrieval as weak decayed attention signals. Retrieval also applies a bounded source-quality signal from document/chunk metadata such as `source_type`, `source_trust`, `confidence`, `retrieved_at`, and `freshness_expires_at`, so trusted local sources and fresh confident findings can outrank stale or low-confidence derived material when base relevance is close. Remaining drawback: ranking still does not yet use explicit hot/warm/cold state beyond transcript hotness, active project/session context, learned attention, or a mature time-decay policy.
